"""
CrossEncoder wrapper for OpenProvenceEncoder models.
"""
...
try:
    ...
    from .utils.sentence_transformers_compat import CrossEncoder
from .encoder import OpenProvenceEncoder

logger = logging.getLogger(__name__)


class OpenProvenceCrossEncoder(CrossEncoder):
    """
    CrossEncoder wrapper for OpenProvenceEncoder reranking models.
    
    This allows OpenProvenceEncoder models to be used as drop-in replacements for CrossEncoder.
    """
    
    def __init__(self, model_name_or_path: str, **kwargs):
        """
        Initialize OpenProvenceCrossEncoder.
        
        Args:
            model_name_or_path: Path to OpenProvenceEncoder model
            **kwargs: Additional arguments (ignored for compatibility)
        """
        # Skip parent init to avoid loading with AutoModel
        torch.nn.Module.__init__(self)
        
        # Load OpenProvenceEncoder
        self.model = OpenProvenceEncoder.from_pretrained(model_name_or_path)
        
        if self.model.mode != "reranking_pruning":
            raise ValueError(
                f"OpenProvenceCrossEncoder only supports reranking_pruning mode, "
    ...
    def predict(
        ...
        **kwargs
        ...
        """
        # Use OpenProvenceEncoder's predict method

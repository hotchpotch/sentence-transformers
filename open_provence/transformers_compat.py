"""
Transformers library compatibility wrappers for OpenProvenceEncoder.
...
class OpenProvenceEncoderConfig(PretrainedConfig):
    """Configuration class for OpenProvenceEncoder models."""
    
    model_type = "openprovence_encoder"
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize OpenProvenceEncoderConfig.
...
class OpenProvenceEncoderForSequenceClassification(PreTrainedModel):
    """
    OpenProvenceEncoder wrapper for sequence classification (reranking).
    Compatible with AutoModelForSequenceClassification.
    """
    
    config_class = OpenProvenceEncoderConfig
    base_model_prefix = "pruning_encoder"
    
    def __init__(self, config: OpenProvenceEncoderConfig):
        super().__init__(config)
        
        # Initialize OpenProvenceEncoder
        self.pruning_encoder = OpenProvenceEncoder(
    ...
    def forward(
        ...
        **kwargs
        ...
        """
        # Forward through OpenProvenceEncoder with correct arguments
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        """Load from a OpenProvenceEncoder checkpoint."""
        # First try to load config
        config_path = os.path.join(pretrained_model_name_or_path, "config.json")
        if os.path.exists(config_path):
            config = OpenProvenceEncoderConfig.from_pretrained(pretrained_model_name_or_path)
        else:
            # Create config from OpenProvenceEncoder's saved config
            ...
            if os.path.exists(pruning_config_path):
                ...
                config = OpenProvenceEncoderConfig(
        ...
        # Load weights from OpenProvenceEncoder
        model.pruning_encoder = OpenProvenceEncoder.from_pretrained(
...
class OpenProvenceEncoderForTokenClassification(PreTrainedModel):
    """
    OpenProvenceEncoder wrapper for token classification (pruning).
    ...
    def __init__(self, config: OpenProvenceEncoderConfig):
        ...
        # Initialize OpenProvenceEncoder in pruning_only mode
        self.pruning_encoder = OpenProvenceEncoder(
            model_name_or_path=config.base_model_name_or_path,
            mode="pruning_only",
            max_length=config.max_length,
...
def register_auto_models():
    """Register OpenProvenceEncoder models with AutoModel."""
    # Register config
    AutoConfig.register("openprovence_encoder", OpenProvenceEncoderConfig)

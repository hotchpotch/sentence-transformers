#!/usr/bin/env python3
"""
Process 100k dataset with only teacher scores.
...
def process_batch(batch, teacher_model, device):
    ...
    for i in range(batch_size):
        query = batch['query'][i]
        texts = batch['texts'][i]
        labels = batch['labels'][i]  # Original dataset uses 'labels' not 'ranking_labels'
...
def main():
    logger.info("Processing 100k dataset with teacher scores only...")
    
    # Paths
    output_path = "tmp/datasets/dev-dataset/small-100k-simple"
    ...
    logger.info("Creating 100k subset...")
    target_size = 100000
    train_data = dataset['train'].select(range(target_size))
    
    # Split into train/val/test (80/10/10)
    train_size = len(train_data)
    train_end = int(0.8 * train_size)  # 80,000
    val_end = int(0.9 * train_size)    # 90,000
    ...
    for split_name, split_data in processed_dataset.items():
        ...
        if len(split_data) > 0:
            ...
            logger.info(f"  Has relevant_chunks: {'relevant_chunks' in sample}")
    
    logger.info("100k dataset processing completed!")

#!/usr/bin/env python3
"""
Process 5k dataset with proper batching for all 5 texts per query.
Smaller dataset for faster experimentation.
"""
...
def main():
    logger.info("Processing 5k dataset with batched approach...")
    
    # Paths
    input_path = "tmp/datasets/dev-dataset/small-100k"
    output_path = "tmp/datasets/dev-dataset/small-5k-batched"
    target_size = 5000  # 5k total
    ...
    # Take first 5k from train, maintaining split ratios
    train_size = int(0.8 * target_size)  # 4k
    val_size = int(0.1 * target_size)    # 500  
    test_size = int(0.1 * target_size)   # 500
    ...
    for split_name, split_data in new_splits.items():
        ...
        all_labels = []
        all_ranking_labels = []
        for example in processed_examples[:100]:  # Check first 100
    ...
    logger.info("5k batched dataset processing completed!")

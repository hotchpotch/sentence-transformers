#!/usr/bin/env python
"""
Train a minimal pruning-only model with the new MS MARCO dataset.
"""
...
def main():
    """Train minimal pruning-only model."""
    logger.info("="*60)
    logger.info("Training Minimal Pruning-Only Model (MS MARCO)")
    logger.info("="*60)
    
    # Output directory
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_dir = f"./output/msmarco_minimal_pruning_only_{timestamp}"

#!/usr/bin/env python3
"""
Train ProvenceEncoder on 100k batched dataset with proper handling of multiple texts per query.
Full scale training.
"""
...
def main():
    ...
    loss_fn = ProvenceBatchedLoss(
        model=model,
        ranking_weight=1.0,
        pruning_weight=1.0,  # Increase pruning weight for 100k
    ...
    training_args = {
        ...
        "learning_rate": 2e-5,  # Slightly lower for larger dataset
        "batch_size": 48,  # 48 queries × 5 texts = 240 pairs
        "num_epochs": 1,  # 1 epoch for 100k is substantial
        "weight_decay": 0.01,
        "logging_steps": 100,
        "eval_steps": 500,
        "save_steps": 1000,

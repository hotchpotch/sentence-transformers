#!/usr/bin/env python
"""
Reranking + Pruning model training script for small dataset.
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f'./log/train_reranking_pruning_small_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
    ]
)
...
def main():
    """Train a reranking + pruning model on small dataset."""
    ...
    config = {
        'num_epochs': 2,  # Same as pruning-only small
        'batch_size': 32,
        'learning_rate': 2e-5,
        'eval_steps': 500,
        'save_steps': 500,
    ...
    # Output directory
    output_dir = f"./output/reranking_pruning_small_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

#!/usr/bin/env python3
"""
CLI tool for text pruning using OpenProvenceEncoder models.
...
def process_json_input(json_file: str, model_path: str, thresholds: List[float], use_majority: bool, batch_size: int):
    ...
    # Load model once
    model = OpenProvenceEncoder.from_pretrained(model_path)
    model.eval()
...
def main():
    ...
    if args.json:
        ...
        model = OpenProvenceEncoder.from_pretrained(args.model)
        model.eval()

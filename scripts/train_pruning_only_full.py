#!/usr/bin/env python
"""
Pruning-only model training script for full dataset.
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f'./log/train_pruning_only_full_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
    ]
)
...
def main():
    """Train a pruning-only model on full dataset."""
    
    dataset_name = 'full'
    logger.info(f"Starting pruning-only training with {dataset_name} dataset")
    
    # Load dataset
    logger.info(f"Loading dataset: hotchpotch/wip-query-context-pruner-with-teacher-scores/ja-full")
    ...
    # Training configuration (aligned with small dataset)
    config = {
        'num_epochs': 1,  # Single epoch like small
        'batch_size': 32,  # Same as small
        'learning_rate': 2e-5,
        'eval_steps': 1000,
        'save_steps': 1000,
        'logging_steps': 100,
        'gradient_accumulation_steps': 2,  # Add gradient accumulation for effective batch size 64
    }
    
    # Output directory
    output_dir = f"./output/pruning_only_full_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

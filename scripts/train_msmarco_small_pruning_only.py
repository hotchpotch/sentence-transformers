#!/usr/bin/env python
"""
Train a small pruning-only model with the MS MARCO dataset.
"""
...
def main():
    """Train small pruning-only model."""
    logger.info("="*60)
    logger.info("Training Small Pruning-Only Model (MS MARCO)")
    logger.info("="*60)
    
    # Output directory
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_dir = f"./output/msmarco_small_pruning_only_{timestamp}"

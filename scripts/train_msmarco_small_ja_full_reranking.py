#!/usr/bin/env python
"""
Train reranking+pruning model on full msmarco-small-ja dataset.
"""
...
def main():
    """Train reranking+pruning model on full msmarco-small-ja."""
    logger.info("="*60)
    logger.info("Training Full Reranking+Pruning Model (MS MARCO Small-JA)")
    logger.info("="*60)
    
    # Output directory
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_dir = f"./output/msmarco_small_ja_full_reranking_pruning_{timestamp}"
    
    # Load dataset
    train_dataset, eval_dataset, test_dataset = load_msmarco_dataset("msmarco-small-ja")
    
    # Use full dataset this time
    logger.info(f"Using FULL dataset - Train: {len(train_dataset)}, Eval: {len(eval_dataset)}")
    ...
    model = PruningEncoder(
        ...
        tokenizer_args={"use_fast": False}  # Use slow tokenizer
    )
    ...
    training_args = {
        ...
        "num_epochs": 1,  # 1 epoch for full dataset
        "batch_size": 4,
        "learning_rate": 2e-5,
        "warmup_ratio": 0.1,
        "weight_decay": 0.01,
        "gradient_accumulation_steps": 4,  # Larger accumulation for full dataset
        "max_grad_norm": 1.0,
        "logging_steps": 100,
        "eval_steps": 1000,
        "save_steps": 2000,

#!/usr/bin/env python
"""
Reranking + Pruning model training script for full dataset.
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f'./log/train_reranking_pruning_full_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
    ]
)
...
def main():
    """Train a reranking + pruning model on full dataset."""
    ...
    config = {
        'num_epochs': 1,  # Same as pruning-only full
        'batch_size': 32,
        'learning_rate': 2e-5,
        'eval_steps': 1000,
        'save_steps': 1000,
        'logging_steps': 100,
        'gradient_accumulation_steps': 2,  # Same as pruning-only full
    }
    
    # Output directory
    output_dir = f"./output/reranking_pruning_full_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

#!/usr/bin/env python
"""
Train a minimal reranking+pruning model with the new MS MARCO dataset.
"""
...
def load_msmarco_dataset(subset: str = "msmarco-minimal-ja"):
...
def main():
    """Train minimal reranking+pruning model."""
    logger.info("="*60)
    logger.info("Training Minimal Reranking+Pruning Model (MS MARCO)")
    logger.info("="*60)
    
    # Output directory
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_dir = f"./output/msmarco_minimal_reranking_pruning_{timestamp}"
    
    # Load dataset
    train_dataset, eval_dataset, test_dataset = load_msmarco_dataset("msmarco-minimal-ja")
    ...
    logger.info(f"Sample texts length: {len(sample['texts'])}")

...
Compare small and full pruning-only models.
"""
...
def main():
    ...
    # Model paths
    small_model_path = "./output/pruning_only_small_20250709_084354/checkpoint-1000-best"
    full_model_path = "./output/pruning_only_full_20250709_091455/checkpoint-7817-best"
    ...
    logger.info(f"\n{'='*60}")
    logger.info("Evaluating Small Pruning-Only Model")
    logger.info(f"{'='*60}\n")
    
    model_small = PruningEncoder.from_pretrained(
        small_model_path,
    ...
    for threshold in thresholds:
        logger.info(f"\nEvaluating small model with threshold={threshold}")
        pos_metrics, neg_metrics, all_metrics = predict_and_evaluate(
            model_small, test_dataset, batch_size=16, threshold=threshold
        )
        
        results['small'][threshold] = {
            'pos': pos_metrics,
            'neg': neg_metrics,
            'all': all_metrics
        }
    ...
    # Evaluate full model
    logger.info(f"\n{'='*60}")
    logger.info("Evaluating Full Pruning-Only Model")
    logger.info(f"{'='*60}\n")
    
    model_full = PruningEncoder.from_pretrained(
        full_model_path,
        device="cuda" if torch.cuda.is_available() else "cpu"
    )
    ...
    for threshold in thresholds:
        logger.info(f"\nEvaluating full model with threshold={threshold}")
        pos_metrics, neg_metrics, all_metrics = predict_and_evaluate(
            model_full, test_dataset, batch_size=16, threshold=threshold
        )
        
        results['full'][threshold] = {
            'pos': pos_metrics,
            'neg': neg_metrics,
            'all': all_metrics
        }
    ...
    logger.info(f"\n{'='*60}")
    logger.info("Small vs Full Model Comparison at threshold=0.5")
    logger.info(f"{'='*60}\n")
    
    threshold = 0.5
    
    logger.info("Metric | Small Model | Full Model | Difference")
    logger.info("-------|-------------|------------|------------")
    
    for metric_type in ['pos', 'neg', 'all']:
        if metric_type == 'pos' and (not results['small'][threshold]['pos'] or 
                                      not results['full'][threshold]['pos']):
            continue
        if metric_type == 'neg' and (not results['small'][threshold]['neg'] or 
                                      not results['full'][threshold]['neg']):
            continue
            
        m_small = results['small'][threshold][metric_type]
        m_full = results['full'][threshold][metric_type]
        
        logger.info(f"{metric_type.upper()} F2  | {m_small['f2']:.4f}      | {m_full['f2']:.4f}     | {m_full['f2'] - m_small['f2']:+.4f}")
        logger.info(f"{metric_type.upper()} Rec | {m_small['recall']:.4f}      | {m_full['recall']:.4f}     | {m_full['recall'] - m_small['recall']:+.4f}")
        logger.info(f"{metric_type.upper()} Pre | {m_small['precision']:.4f}      | {m_full['precision']:.4f}     | {m_full['precision'] - m_small['precision']:+.4f}")
        logger.info("-------|-------------|------------|------------")

#!/usr/bin/env python3
"""
ja-minimal で学習した Provence モデルの評価スクリプト
トークンレベルプルーニングの評価
"""
...
def evaluate_model(model_path: str, dataset_subset: str = 'ja-minimal'):
    ...
    # ランキングスコア予測
    print("  ランキングスコア予測中...")
    ranking_scores = model.predict(all_pairs, batch_size=32, show_progress_bar=True)
    ...
    if positive_items:
        ...
        print(f"  入力トークン数: {encoded['input_ids'].shape[1]}")
        ...
        for threshold in [0.1, 0.3, 0.5, 0.7, 0.9]:
            ...
            kept_tokens = output.num_pruned_sentences  # Actually number of kept tokens
            total_tokens = len(output.sentences[0]) if output.sentences else 0
            print(f"  閾値 {threshold}: {total_tokens - kept_tokens}/{total_tokens} トークン保持 (圧縮率 {output.compression_ratio:.1%})")

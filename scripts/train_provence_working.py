#!/usr/bin/env python3
"""
Train Provence model on minimal dataset - working version.
"""
...
def prepare_dataset_for_training(dataset):
    ...
    return dataset.map(rename_fields, remove_columns=['text', 'ranking_label', 'teacher_score'])
...
def main():
    ...
    dataset = dataset.map(lambda ds: prepare_dataset_for_training(ds), batched=True)

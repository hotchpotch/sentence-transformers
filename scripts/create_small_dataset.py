#!/usr/bin/env python3
"""
Create small dataset (50k samples) from the original dataset.
"""
...
def main():
    logger.info("Creating small dataset (50k samples)...")
    
    # Configuration
    dataset_name = "hotchpotch/wip-query-context-pruner"
    output_path = "tmp/datasets/dev-dataset/small"
    target_size = 50000
    ...
    if 'train' in dataset:
        ...
        if len(train_data) > target_size:
            ...
            train_small = train_data.select(selected_indices)
            logger.info(f"Small train size: {len(train_small)}")
        else:
            train_small = train_data
            logger.info(f"Using full train dataset: {len(train_small)}")
        
        new_splits['train'] = train_small
    ...
    logger.info(f"Saving small dataset to {output_path}")
    small_dataset.save_to_disk(str(output_path))
    
    logger.info("Small dataset created successfully!")

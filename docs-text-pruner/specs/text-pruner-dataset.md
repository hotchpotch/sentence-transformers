...
- **言語**: 日本語・英語のバイリンガルデータセット
- **元データソース**: 複数のQAデータセットから構成

### データセット構成

| データセット名 | サンプル数 | 割合 | 言語 | 説明 |
|--------------|-----------|------|------|------|
| ms-marco | 502,930 | 38.97% | 英語 | Microsoft MARCOデータセット |
| ms-marco-ja | 502,832 | 38.97% | 日本語 | MS MARCOの日本語翻訳版 |
| auto-wiki-qa-nemotron | 147,918 | 11.46% | 日本語 | Wikipedia QAデータセット |
| jsquad | 59,106 | 4.58% | 日本語 | 日本語版SQuAD |
| jaquad | 29,006 | 2.25% | 日本語 | 日本語QAデータセット |
| quiz-no-mori | 19,418 | 1.50% | 日本語 | クイズの森データ |
| quiz-works | 15,828 | 1.23% | 日本語 | クイズ作品データ |
| mkqa-ja | 4,206 | 0.33% | 日本語 | 多言語知識QA（日本語版） |
| mr-tydi | 3,582 | 0.28% | 日本語 | 多言語情報検索データセット |
| miracl-ja | 3,464 | 0.27% | 日本語 | 多言語情報検索チャレンジ |
| jqara | 2,129 | 0.16% | 日本語 | 日本語QARAデータセット |
...
2. **バイリンガルデータ**: 日本語（約61%）と英語（約39%）の混在データセット
3. **ラベルの品質**: Silver labelsのため、完全な精度は保証されません
4. **データサイズ**: 約130万サンプルと大規模なため、開発時はサブセットの使用を推奨
5. **MS MARCO重視**: 全体の約78%がMS MARCO由来のデータ（原文と翻訳版）

## バイリンガルモデルの利点

1. **クロスリンガル能力**: 日本語クエリで英語文書を検索（またはその逆）が可能
2. **汎用性**: 単一モデルで複数言語に対応
3. **MS MARCOの活用**: 高品質な英語QAデータとその翻訳版を活用
4. **多様なドメイン**: クイズ、Wikipedia、技術文書など多様なソースから学習

## 今後の拡張計画

1. **言語バランスの調整**: 日本語・英語の比率最適化
2. **ドメイン拡張**: 技術文書、ニュース記事などの追加
3. **Hard negative mining**: より困難な負例の追加
4. **品質向上**: Gold labelsの追加、人手によるアノテーション
5. **他言語への拡張**: 中国語、韓国語などアジア言語の追加
